...
namespace ToolBox.Extensions {
    internal static class XmlExtensions
    {
        ...
        public static void WriteValue(this XmlTextWriter self, string prefix, AnimationCurve value)
        {
            self.WriteStartElement(prefix);
            foreach (Keyframe key in value.keys)
            {
                self.WriteStartElement("curveKeyframe");
                self.WriteAttributeString("time", XmlConvert.ToString(key.time));
                self.WriteAttributeString("value", XmlConvert.ToString(key.value));
                self.WriteAttributeString("inTangent", XmlConvert.ToString(key.inTangent));
                self.WriteAttributeString("outTangent", XmlConvert.ToString(key.outTangent));
                self.WriteEndElement();
            }
            self.WriteEndElement();
        }

        public static AnimationCurve ReadAnimationCurve(this XmlNode self, string prefix)
        {
            AnimationCurve curve = new AnimationCurve();
            foreach (XmlNode curveKeyframe in self.FindChildNode(prefix).ChildNodes)
            {
                curve.AddKey(new Keyframe(
                    XmlConvert.ToSingle(curveKeyframe.Attributes["time"].Value),
                    XmlConvert.ToSingle(curveKeyframe.Attributes["value"].Value),
                    XmlConvert.ToSingle(curveKeyframe.Attributes["inTangent"].Value),
                    XmlConvert.ToSingle(curveKeyframe.Attributes["outTangent"].Value)
                ));
            }
            return curve;
        }
